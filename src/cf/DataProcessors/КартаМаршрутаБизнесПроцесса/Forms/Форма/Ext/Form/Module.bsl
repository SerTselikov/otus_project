...
Процедура ОбновитьКартуМаршрута()
	...
	СвойстваБизнесПроцесса = БизнесПроцесс;
...
#КонецЕсли
	...
	Если Не ЗначениеЗаполнено(БизнесПроцесс) Тогда
		Сообщить(НСтр("ru = 'Укажите бизнес-процесс.'"));
	...
	Если ТекЭлемент = Неопределено 
		...
		Сообщить(НСтр("ru = 'Для просмотра списка задач выберите точку действия или вложенный бизнес-процесс карты маршрута.'"));
		//ОбщегоНазначенияКлиент.СообщитьПользователю(
